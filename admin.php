...
if ($IN['login'] != 'yes') {
	...
	if ( (!$IN['adsess']) or (empty($IN['adsess'])) or (!isset($IN['adsess'])) or ($IN['adsess'] == "") )
	{
		...
		do_login("Сеансов администрирования не обнаружено");
	}
	else
	{
		//----------------------------------
		...
		if ($row['ID'] == "")
		{
			...
			do_login("Невозможно найти записи сессии");
		}
		else if ($row['MEMBER_ID'] == "")
		{
			...
			do_login("Невозможно определить пользователя с этим id");
		...
		else
		{
			...
			if ($MEMBER['id'] == "")
			{
				...
				do_login("Неверный ID пользователя");
			...
			else
			{
				...
				else
				{
					...
					{
						do_login("У Вас нет прав входа в Админцентр");
					}
					else
					{
						$session_validated = 1;
...
else 
{
	...
	{
		do_login("Для авторизации необходимо ввести имя пользователя");
	}
	
	if ( empty($IN['password']) )
	{
		do_login("Для авторизации необходимо ввести пароль");
	}
	...
	if ( empty($mem['id']) )
	{
		do_login("В базе данных нет пользователя с таким именем. Проверьте раскладку клавиатуры.");
	}
	...
	{
		do_login("Введённый Вами пароль недействителен");
	}
	else
	{
		...
		else
		{
			...
			$ADMIN->page_title = "Успешная авторизация";
			
			$ADMIN->page_detail = "Сейчас Вы будете перемещены в панель управления Администратора";
			
			$ADMIN->html .= $SKIN->start_table("В процессе");
			
			$ADMIN->html .= "<tr><td id='tdrow1'><meta http-equiv='refresh' content='2; url=".$INFO['board_url']."/admin.".$INFO['php_ext']."?adsess=".$IN['AD_SESS']."'><a href='".$INFO['board_url']."/admin.".$INFO['php_ext']."?adsess=".$IN['AD_SESS']."'>( Нажмите сюда, если не хотите ждать )</a></td></tr>";
...
if ($session_validated == 1)
{
	...
	{
		$session_validated = 0;
		do_login("Ваш сеанс администрирования истёк");
	}
	...
	else if ($check_ip == 1)
	{
		...
		{
			$session_validated = 0;
			do_login("Ваш текущий IP адрес не соответствует ни одному из IP адресов");
		}
	}
}
...
else
{
	...
	do_login("Неверная сессия. Необходима переавторизация.");
...
function do_login($message="") {
	...
	$ADMIN->page_detail = "Вы должны иметь права Администратора, для входа в Админцентр форума.<br>Ниже введите Ваше имя пользователя и пароль";
	...
	$ADMIN->html .= $SKIN->start_table( "Требование авторизации" );
	
	$ADMIN->html .= $SKIN->add_td_row( array( "Ваше имя:",
											  "<input type='text' style='width:100%' name='username' value='$name'>",
									 )      );
		
	$ADMIN->html .= $SKIN->add_td_row( array( "Ваш пароль:",
											  "<input type='password' style='width:100%' name='password' value=''>",
									 )      );
									 
	$ADMIN->html .= $SKIN->end_form("Войти");
