...
class post_parser {
	...
	function unconvert($txt="", $code=1, $html=0) {
		...
		if ($code == 1)
		{
			...
			$txt = preg_replace( "#<!--SpoilerBegin-->(.+?)<!--SpoilerEBegin-->#"                , '[SPOILER]'         , $txt );
			$txt = preg_replace( "#<!--SpoilerEnd-->(.+?)<!--SpoilerEEnd-->#"                    , '[/SPOILER]'        , $txt );
	...
	function regex_sql_tag($sql="") {
       ...
       /**************************************************/
        ...
        function regex_exclaime_tag($txt="") {
                ...
                $html = "<!--excl1--></div><BR><TABLE><TD align=middle bgColor=#ff6060 vAlign=center width=1%><FONT color=#ffffff size=+7 style=\"FONT-FAMILY: Times\" title=\"Предупреждение от модератора\"><B>&nbsp;!&nbsp;</B></FONT></TD><TD width=3><BR></TD><TD vAlign=top><!--eexcl1-->".$txt."<!--excl2--></TD></TABLE><div class='postcolor'><!--eexcl2-->";
...
/****************************************************************************************************/
	// regex_parse_spoiler: Builds this quote tag HTML
	// [SPOILER] .. [/SPOILER]
	/**************************************************/
	...
	/**************************************************/
	// regex_simple_spoiler_tag: Builds this spoiler tag HTML
	// [SPOILER] .. [/SPOILER]
	/**************************************************/
	
	function regex_simple_spoiler_tag() {
		global $ibforums;
		
		$this->spoiler_open++;

		return "<!--SpoilerBegin-->{$this->spoiler_html['START']}<!--SpoilerEBegin-->";
		
	}
	
	/**************************************************/
	// regex_close_spoiler: closes a spoiler tag
	// 
	/**************************************************/
	
	function regex_close_spoiler() {
	
		if ($this->spoiler_open == 0) {
			$this->spoiler_error++;
		 	return;
		 }
		 ...
		 return "<!--SpoilerEnd-->{$this->spoiler_html['END']}<!--SpoilerEEnd-->";
	}
