...
$DB->query( "SELECT * FROM ibf_topics ORDER BY posts DESC LIMIT 0,7");
$TPosts="";
while($tposts=$DB->fetch_row()){
$ob_tltl = substr($tposts['title'], 0, 30);
$TPosts.="<tr><td>» <a href='index.php?showtopic={$tposts['tid']}' onClick=\"this.href='index.php?act=ST&f={$tposts['forum_id']}&t={$tposts['tid']}&view=getlastpost'\">$ob_tltl</a> (<b><font color=#3a4f6c>{$tposts['posts']}</font></b>) {$tposts['last_poster_name']}</td></tr>";;}

$DB->query("SELECT * FROM ibf_members WHERE id>0 ORDER BY `joined` DESC LIMIT 0,7");
$TNew="";
while($tnew=$DB->fetch_row()){
$TNew.="<tr><td>» <i>{$tnew['name']}</i> ({$tnew['posts']})</td></tr>";}

$DB->query( "SELECT * FROM ibf_topics ORDER BY last_post DESC LIMIT 0,7");
$ToNew="";
while($tonew=$DB->fetch_row()){
$ob_tltl = substr($tonew['title'], 0, 30);
$ToNew.="<tr><td>» <a href='index.php?showtopic={$tonew['tid']}' onClick=\"this.href='index.php?act=ST&f={$tonew['forum_id']}&t={$tonew['tid']}&view=getlastpost'\">$ob_tltl</a> (<b><font color=#3a4f6c>{$tonew['posts']}</font></b>)";
$ToNew.=" {$tonew['last_poster_name']}</td></tr>";}

$this->output = str_replace( "<!-- STATPANEL -->" , $this->html->Top_Five_Stats($TPosts,$TNew,$ToNew) , $this->output );
