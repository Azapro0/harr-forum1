...
class ad_plugins {
	...
	function copy_splash()
	{
		...
		{
			$frames = "<html>
		   			 <head><title>Invision Power Board: Регистрация</title></head>
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
	...
	function copy_frame()
	{
		...
		$html = "<html>
				  <head>
				   <title>Окно заказа Invision Power Board</title>
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
				   $css
				  </head>
				  ...
				  <tr>
				   <td valign='middle' align='left'><b style='color:white'>Уже уплачено за удаление копирайта?</b></td>
				   <td valign='middle' align='left'><input type='text' size=50 name='ipb_copy_number' value='введите Ваш ключ удаления копирайта IPB...' onClick=\"this.value='';\"></td>
				   <td valign='middle' align='left'><input type='submit' value='Далее...'></td>
				  </tr>
				  </table>
				  </form>
				  </body>
	...
	function copy_save()
	{
		...
		if ( stristr( $acc_number, ',pass=' ) )
		{
			...
			{
				$ADMIN->error("Пароль неправильный. Вы можете обратиться в <a href='http://www.invisionpower.com/?contact'>службу поддержки</a>, за помощью или получите новый чек, с Вашего <a href='http://customer.invisionpower.com'>аккаунта заказчика IPS</a>.");
			}
		}
		
		if ( $acc_number == "" )
		{
			$ADMIN->error("Это неправильный ключ IPB копирайта, вернитесь назад и попробуйте снова.");
		}
		...
		if ( $response == "" )
		{
			$ADMIN->error("Невозможно получить ответ с сервера регистрации Invision Power Services, что могло произойти по следующим причинам:
			               <ul>
			               <li>Ваша версия PHP, не поддерживает удалённые связи</li>
			               <li>Сервер регистрации Invision Power Services, временно отключен</li>
			               <li>Вы запустили IPB, не подключившись в интернет</li>
			               </ul>
			               <br />
			               Вы можете обратиться в <a href='http://www.invisionpower.com/?contact'>службу поддержки</a> , за помощью или получите новый чек, с Вашего <a href='http://customer.invisionpower.com'>аккаунта заказчика IPS</a>.
			             ");
		}
		else if ( $response == '0' )
		{
			$ADMIN->error("Введённый Вами ключ регистрации, неверный, что могло произойти по следующим причинам:
			               <ul>
			               <li>Вы неправильно ввели ключ регистрации</li>
			               <li>Вы использовали свой пароль клиента, вместо ключа регистрации</li>
			               <li>Ваша лицензия регистрации истекла</li>
			               </ul>
			               <br />
			               Вы можете обратиться в <a href='http://www.invisionpower.com/?contact'>службу поддержки</a> , за помощью или получите новый чек, с Вашего <a href='http://customer.invisionpower.com'>аккаунта заказчика IPS</a>.
			             ");
	...
	function copy_config($type="")
	{
		...
		$ADMIN->page_detail = "&nbsp;";
		$ADMIN->page_title  = "Подтверждение IPB копирайта";
		
		if ( $type == "new" )
		{
			$ADMIN->page_detail .= "<br /><br /><b style='color:red'>Благодарим Вас за регистрацию, для удаления копирайта</b>";
		}
		...
		$ADMIN->html .= $SKIN->start_table( "Конфигурация" );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "Копирайт теперь удалён, с Ваших страниц форума.<br /><br />Если удаления не последовало, немедленно обратитесь в нашу службу продаж."
								 )      );
	...
	function copy_config_save()
	{
		...
		$ADMIN->done_screen("Конфигурация регистрации IPB обновлена", "Конфигурация регистрации IPB", "act=pin&code=reg" );
	}
	...
	function reg_splash()
	{
		...
		{
			$frames = "<html>
		   			 <head><title>Invision Power Board: Регистрация</title>
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
</head>
					   <frameset rows='*,50' frameborder='yes' border='1' framespacing='0'>
...
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
				  ...
				  <tr>
				   <td valign='middle' align='left'><b style='color:white'>Уже зарегистрировали?</b></td>
				   <td valign='middle' align='left'><input type='text' size=50 name='ipb_reg_number' value='введите ключ регистрации IPB...' onClick=\"this.value='';\"></td>
				   <td valign='middle' align='left'><input type='submit' value='Далее...'></td>
				  </tr>
				  </table>
				  </form>
	...
	function reg_save()
	{
		...
		if ( $acc_number == "" )
		{
			$ADMIN->error("Это неправильный ключ регистрации IPB, вернитесь назад и попробуйте снова.");
		}
	...
	function reg_config($type="")
	{
		...
		$ADMIN->page_detail = "Ниже Вы можете отредактировать конфигурацию";
		$ADMIN->page_title  = "Конфигурация регистрации IPB";
		
		if ( $type == "new" )
		{
			$ADMIN->page_detail .= "<br /><br /><b style='color:red'>Благодраим за регистрацию!</b>";
		}
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Отображать строку регистрации?</b>" ,
										  $SKIN->form_yes_no( "ipb_reg_show", $INFO['ipb_reg_show']  )
								 )      );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Отображать в виде, - зарегистрирована на...</b><br>Пример: <em>Matthew Mecham</em>, <em>IPS, Inc.</em>" ,
										  $SKIN->form_input( "ipb_reg_name", $INFO['ipb_reg_name'] )
								 )      );
										 									 
		$ADMIN->html .= $SKIN->end_form('Сохранить конфигурацию');
	...
	function chat_splash()
	{
		...
		{
			$frames = "<html>
		   			 <head><title>Invision Power Board: Установка чата</title>
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
</head>
					   <frameset rows='*,50' frameborder='yes' border='1' framespacing='0'>
...
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
				  ...
				  <tr>
				   <td valign='middle' align='left'><b style='color:white'>Вы приобрели IP чат?</b></td>
				   <td valign='middle' align='left'><input type='text' size=35 name='account_no' value='введите полученный номер аккаунта...' onClick=\"this.value='';\"></td>
				   <td valign='middle' align='left'><input type='submit' value='Далее...'></td>
				  </tr>
				  </table>
				  </form>
	...
	function chat_save()
	{
		...
		if ( $acc_number == "" )
		{
			$ADMIN->error("Неверный номер аккаунта IP чата");
		}
	...
	function chat_config_save()
	{
		...
		$ADMIN->done_screen("Конфигурация IP чата обновлена", "Конфигурация IP чата", "act=pin&code=ipchat" );
	}
	...
	function chat_config()
	{
		...
		$ADMIN->page_detail = "Ниже Вы можете отредактировать конфигурацию";
		$ADMIN->page_title  = "Конфигурация IP чата";
		...
		$display = array( 0 => array( 'self', 'На странице форума' ),
						  1 => array( 'new',  'В новом окне' ),
						);
		...
		if ( $INFO['chat_account_no'] )
		{
			...
			{
				$server_name = "Неудачный автопоиск. <a href='http://www.invisionboard.com/acp/chatcheck.php?id={$INFO['chat_account_no']}'>Нажмите сюда, для проверки вручную</a>";
			}
			...
			$server_name = "Обновлён: ". $ADMIN->get_date( time(), 'SHORT' );
		}
		...
		$ADMIN->html .= $SKIN->start_table( "Основная настройка" );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Номер аккаунта чата?</b><br>При удалении этого номера, будут удалены ссылки чата и все настройки чата." ,
										  $SKIN->form_input( "chat_account_no", $INFO['chat_account_no'] )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Сервер чата?</b><br>$server_name" ,
										  $SKIN->form_input( "chat_server_addr", $INFO['chat_server_addr'] )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Включить список присутствующих в чате?'</b><br />".$SKIN->form_checkbox( 'chat_hide_whoschatting', $INFO['chat_hide_whoschatting'] ). " Скрывать список присутствующих в чате, если в чате никого нет?" ,
										  $SKIN->form_yes_no( "chat_who_on", $INFO['chat_who_on']  ) . '<br />'
										  . 'Обновлять списко не менее чем через каждые '
										  . $SKIN->form_dropdown( 'chat_who_save', array( 
										  						...
										  						) . ' минут<br />Обновлять хиты посещений: '.$hits.'<br />До какой даты отображать хиты: '.$expire
										  	
										  )      );
										  
		if ( $INFO['chat_pass_md5'] )
		{
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Изменить пароль администрирования IP чата?</b><br>Если хотите изменить пароль администрирования IP чата, введите новый пароль здесь. Не заполняйте это поле, если не хотите изменять пароль." ,
													  $SKIN->form_input( "chat_pass_md5" )
											 )      );
		}
		else
		{
			$ADMIN->html .= $SKIN->add_td_row( array( "<b style='color:red'>ВВЕДИТЕ ПАРОЛЬ АДМИНИСТРИРОВАНИЯ IP ЧАТА</b><br>Для просотра списка присутсвующих в чате, введите пароль администрирования чата." ,
													  $SKIN->form_input( "chat_pass_md5" )
											 )      );
		}
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Разрешить гостям вход в чат?</b><br>При выборе 'нет', для входа в чат, будет требоваться авторизация." ,
										  $SKIN->form_yes_no( "chat_allow_guest", $INFO['chat_allow_guest']  )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Размеры окна чата (ШИРИНА)?</b>" ,
										  $SKIN->form_input( "chat_width", $INFO['chat_width'] ? $INFO['chat_width'] : 600 )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Размеры окна чата (ВЫСОТА)?</b>" ,
										  $SKIN->form_input( "chat_height", $INFO['chat_height'] ? $INFO['chat_height'] : 350 )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Заполнение Pop-up окна чата? (в пикселах)</b><br />Окно чата будет открываться без прокрутки (скроллинга)" ,
										  $SKIN->form_input( "chat_poppad", $INFO['chat_poppad'] ? $INFO['chat_poppad'] : 50 )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Язык интерфейса чата, по умолчанию?</b>" ,
										  $SKIN->form_dropdown( "chat_language", $larray, $language  )
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Загружать чат...?</b>" ,
		...
		if ( $INFO['chat_who_on'] and $INFO["chat_pass_md5"] )
		{
			...
			$ADMIN->html .= $SKIN->start_table( "Кто в чате?" );
			...
			{
				$ADMIN->html .= $SKIN->add_td_row( array( "В настоящее время в чате никого нет" ) );
			}
			
			$ADMIN->html .= $SKIN->add_td_basic( "<a href='{$ADMIN->base_url}&act=pin&code=dorefreshchat'>Произвести обновление</a>", 'right', 'catrow2' );
		...
		$SKIN->td_header[] = array( "Доступ"    , "25%" );
		$SKIN->td_header[] = array( "Администратор"     , "25%" );
		
		//+-------------------------------
		
		$ADMIN->html .= $SKIN->start_table( "Параметры административного доступа" );
		...
		while ( $r = $DB->fetch_row() )
		{
			...
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Параметры доступа'<span style='color:$mode'>{$r['g_title']}</span>' группы...</b>" ,
												  "<strong>Могут пользоваться чатом</strong>&nbsp;".$SKIN->form_checkbox( "sa_{$r['g_id']}", $access[ $r['g_id'] ]  ? 1 : 0 ),
												  "<strong>Администрирование чата</strong>&nbsp;".$SKIN->form_checkbox( "sg_{$r['g_id']}", $allowed[ $r['g_id'] ] ? 1 : 0 ),
									     	)      );
		}
	...
	function common_header( $formcode = "", $section = "", $extra = "" )
	{
		...
		$ADMIN->page_detail = $extra . "Проверьте введённый данные, перед подтверждением изменений";
		$ADMIN->page_title  = "Конфигурация плагина [ $section ]";
