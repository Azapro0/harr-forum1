...
class admin_functions {
	...
	var $img_url;
	var $page_title  = "Добро пожаловать в панель Администрирования Форума";
	var $page_detail = "Из этой панели управления, Вы можете производить все установки и настройки Вашего форума.<br><br>Нажмите на одну из ссылок в левом фрейме для обзора и доступных настроек в этой категории администрирования. В каждой опции содержится дальнейшая информация о конфигурировании и т.д.";
	var $html;
	var $errors = "";
	var $nav    = array();
	var $time_offset = 0;
	...
	function copy_dir($from_path, $to_path, $mode = 0777)
	{
		...
		if ( ! is_dir($to_path) )
		{
			...
			{
				$this->errors = "Невозможно создать директорию '$to_path' проверьте атрибуты CHMOD и попробуйте снова";
				return FALSE;
			}
			else
			{
	...
	function rebuild_config( $new = "" )
	{
		...
		if (! is_array($new) )
		{
			$ADMIN->error("Произошла ошибка при процессе перезаписи config файла форума, процесс приостановлен");
		}
		...
		{
			$ADMIN->error("Грубая ошибка: Невозможно произвести запись в файл conf_global - изменения не сохранены. Попробуйте изменить атрибут CHMOD на 0777");
		}
...
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
	...
	function output() {
		...
		$navigation = array( "<a href='{$this->base_url}&act=index' target='body'>Главная страница AЦ</a>" );
	...
	function error($error="", $is_popup=0) {
		...
		$this->html .= "<div class='tableborder'>
		                 <div class='maintitle'>Сообщение админцентра</div>
	...
	function done_screen($title, $link_text="", $link_url="") {
		...
		$this->page_detail = "Действие успешно выполнено";
		
		$SKIN->td_header[] = array( "&nbsp;"  , "100%" );
		
		$this->html .= $SKIN->start_table("Результат");
		
		$this->html .= $SKIN->add_td_basic( "<a href='{$this->base_url}&{$link_url}' target='body'>Перейти в: $link_text</a>", "center" );
		
		$this->html .= $SKIN->add_td_basic( "<a href='{$this->base_url}&act=index' target='body'>Переход на: Главную Страницу Админцентра</a>", "center" );
	...
	function info_screen($text="", $title='Предупреждение о Safe Mode') {
		global $IN, $INFO, $DB, $std, $SKIN;
		
		$this->page_title  = $title;
		$this->page_detail = "Имейте ввиду:";
	...
	function build_jump_menu()
	{
		...
		$html .= "</select>&nbsp;<input type='button' class='jmenubutton' value='ОК!' onclick='dojump();' />\n</form>";
