...
class ad_langs {
	...
	function remove()
	{
		...
		if ($IN['id'] == "")
		{
			$ADMIN->error("Вы не выбрали tar-архив для распаковки!");
		}
		...
		{
			$ADMIN->error("Невозможно удалить этот файл. Проверьте атрибут CHMOD");
		}
	...
	function import()
	{
		...
		{
			$ADMIN->error( "Данный архив не найден на сервере. Возможно кто-то из администраторов удалил его.");
		}
		...
		switch($IN['type'])
		{
			...
			//---------
			default:
				$ADMIN->error("Неправильный тип архива");
				break;
		}
	...
	function set_import()
	{
		...
		{
			$ADMIN->error("Невозможно произвести запись в директорию $images_dir . Проверьте атрибут этой директории и установите CHMOD для записи. Скрипт форума не в состоянии сделать это самостоятельно.");
		}
		
		if (! is_dir($skins_dir) )
		{
			$ADMIN->error("Невозможно произвести запись в директорию $skins_dir .");
		}
		
		if (! is_writeable($skins_dir) )
		{
			$ADMIN->error("Невозможно произвести запись в директорию $skins_dir . Проверьте атрибут этой директории и установите CHMOD для записи. Скрипт форума не в состоянии сделать это самостоятельно.");
		}
		...
		{
			$ADMIN->error("Этот tar-архив оказался битым архивом. Перезагрузите его в режиме binary и повторите попытку.");
		}
		...
		{
			$ADMIN->error("Произошла ошибка при создании директории скина. Прорьте атрибуты директории 'archive_in'");
		}
		...
		{
			$this->import_error("Невозможно создать новую директорию в директории style_images", $next_id);
		}
		...
		{
			$this->import_error("Невозможно создать новую директорию в директории Skin", $next_id);
		}
		...
		{
			$this->import_error("Невозможно произвести чтение из CSS файла. Проверьте атрибуты этого файла и повторите попытку.", $next_id);
		}
		...
		{
			$this->import_error("Невозможно произвести чтение файла шаблона форума. Проверьте атрибуты этого файла и повторите попытку.", $next_id);
		}
		...
		{
			$this->import_error("Невозможно произвести распаковку изображений. Распаковка приостановлена", $next_id);
		}
		...
		{
			$this->import_error("Невозможно произвести чтение файла macro.txt, который находится в архиве скина.", $next_id);
		}
		...
		{
			$this->import_error("Невозможно произвести чтение файла templates.html, который находится в архиве скина.", $next_id);
		}
		...
		$ADMIN->done_screen("Скин распакован", "Настройка скинов", "act=sets" );
	...
	function process_template_group($raw, $setid, $group, $isnew=0)
	{
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			if ($isnew == 1)
			{
				...
				{	
					$DB->query("DELETE FROM ibf_tmpl_names WHERE skid='$setid'");
					$ADMIN->error("Невозможно произвести запись в директорию 'Skin'. Проверьте атрибуты этой директории, установите при необходимости CHMOD на 0777 и повторите попытку.");
				}
			...
			else
			{
				...
				{
					$ADMIN->error("Невозможно произвести запись в директорию '$skin_dir'. Установите через FTP атрибут на запись - CHMOD 0777. Скрипт форума не с состоянии сделать это автоматически.");
				}
			}
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			else
			{
				...
				$errors[] = "Невозможно сохранить информацию в $phpskin. Проверьте правильность установленных атрибутов CHMOD.";
			}
	...
	function list_current()
	{
		...
		$ADMIN->page_detail = "В этой секции, Вы можете выбрать архив скина для распаковки его в Ваш форум. Архивы необходимо загрузить в директорию 'archive_in'.";
		$ADMIN->page_title  = "Импорт скинов";
		...
		$SKIN->td_header[] = array( "Тип"         , "20%" );
		$SKIN->td_header[] = array( "Файл"     , "30%" );
		$SKIN->td_header[] = array( "Распаковка"       , "10%" );
		$SKIN->td_header[] = array( "Удаление"       , "10%" );
		
		$ADMIN->html .= $SKIN->start_table( "Загруженные архивы" );
		...
		if ( count($files) > 0 )
		{
			foreach($files as $file)
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array( "<b>$rname</b>",
														  ...
														  "<center>$file</center>",
														  "<center><a href='".$SKIN->base_url."&act=import&code=import&type=$rtype&id=$file'>Распаковать</a></center>",
														  "<center><a href='".$SKIN->base_url."&act=import&code=remove&id=$file'>Удалить</a></center>",
												 )      );
			}
	...
	function rebuild_phpskin($templates_dir, $skins_dir)
	{
			...
			while (($filename = readdir($handle)) !== false)
			{
				if (($filename != ".") && ($filename != ".."))
				{
					...
					if ( preg_match( "/\.html$/", $filename ) )
					{
						...
						{
							$errors[] = "Невозможно открыть файл $filename...";
							continue;
						}
						...
						if ($need < 1)
						{
							$errors[] = "Ошибка при вызове данных файла $filename ...";
							continue;
						}
						...
						if ($start != $end)
						{
							$errors[] = "В некоторых комментариях файла шаблона $filename , найдены ошибочные данные ...";
							continue;
						}
		...
		{
			$errors[] = "Невозможно открыть директорию шаблонов!";
		}
