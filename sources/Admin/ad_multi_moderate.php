...
class ad_multimod
{
	...
	function ad_multimod()
	{
		...
		$ADMIN->nav[] = array( 'act=multimod', 'Главная страница мульти-модерации тем' );
	...
	function do_delete()
	{
		...
		if ($IN['id'] == "")
		{
			$ADMIN->error("Невозможно определить ID мульти-модерации. Попробуйте снова.");
		}
		...
		$ADMIN->save_log("Удаление мульти-модерации темы");
	...
	function do_save($type='new')
	{
		...
		if ( $type == 'edit' )
		{
			if ( $IN['id'] < 1 )
			{
				$ADMIN->error("Вы должны использовать правильный id");
			}
		}
		
		if ( $IN['mm_title'] == "" )
		{
			$ADMIN->error("Необходимо ввести правильное название");
		}
		...
		if ( count($forums) < 1 )
		{
			$ADMIN->error("Необходимо выбрать форумы, для активации в них, функции мульти-модерации");
		}
		
		if ( $IN['topic_move'] == 'n' )
		{
			$ADMIN->error("Неправильно выбран форум в секции 'Перемещение в' для мульти-модерации тем. Имейте ввиду, что тему невозможно переместить в категорию.");
		}
		...
		$ADMIN->save_log("Обновление правил мульти-модерации ($type)");
	...
	function do_form($type='new')
	{
		...
		$ADMIN->page_detail = "Мульти-модерация тем.";
		$ADMIN->page_title  = "Создание правила мультимодерации тем";
		
		$form_code   = 'donew';
		$description = 'Создание нового правила мульти-модерации тем';
		$button      = "Создать мульти-модерацию";
		if ( $type == 'edit' )
		{
			$id = intval($IN['id']);
			...
			{
				$ADMIN->error("Невозможно извлечь информацию ($id)");
			}
			
			$form_code   = 'doedit';
			$description = 'Редактирование правила мультио-модерации тем';
		...
		$pin_dd   = array(
						  0 => array( 'leave', 'Не изменять' ),
						  1 => array( 'pin'  , 'Зафиксировать'   ),
						  2 => array( 'unpin', 'Опустить' ),
					    );
		...
		$ADMIN->html .= $SKIN->start_table( "Мульти-модерация тем", $description );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Название этого правила мульти-модерации?</b>" ,
												  $SKIN->form_input("mm_title", $topic_mm['mm_title'] )
									     )      );
		
									     
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Задействовать в форумах...</b><br>Можно выбрать несколько форумов сразу, удерживая клавишу <b>Ctrl</b>" ,
												  $forums
									     )      );							     
		
		$ADMIN->html .= $SKIN->add_td_basic( 'Опции модерации', 'left', 'pformstrip' );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Добавлять в <i>НАЧАЛО</i> названия темы?</b>" ,
												  $SKIN->form_input("topic_title_st", $topic_mm['topic_title_st'] )
									     )      );
									     
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Добавлять в <i>КОНЕЦ</i> названия темы?</b>" ,
												  $SKIN->form_input("topic_title_end", $topic_mm['topic_title_end'] )
									     )      );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Изменять состояние темы?</b>" ,
												  $SKIN->form_dropdown("topic_state", $state_dd, $topic_mm['topic_state'] )
									     )      );
									     
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Изменять состояние фиксации?</b>" ,
												  $SKIN->form_dropdown("topic_pin", $pin_dd, $topic_mm['topic_pin'] )
									     )      );
									     
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Перемещать тему?</b>" ,
					    						  $SKIN->form_dropdown("topic_move", array_merge( array( 0 => array('-1', 'Не перемещать' ) ), $forum_jump ), $topic_mm['topic_move'] )
					    						  ."<br />".$SKIN->form_checkbox('topic_move_link', $topic_mm['topic_move_link'] )."<strong>Оставлять ссылки на перемещённые темы в исходном форуме?</strong>"
									     )      );
		
		$ADMIN->html .= $SKIN->add_td_basic( 'Опции сообщения', 'left', 'pformstrip' );
	
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Добавлять ответ в тему?</b><br>HTML разрешён" ,
												  "Включить добавление этого ответа? &nbsp;".$SKIN->form_yes_no('topic_reply', $topic_mm['topic_reply'] )
												  ."<br />"
												  . $SKIN->form_textarea("topic_reply_content", $topic_mm['topic_reply_content'] )
												  ."<br />".$SKIN->form_checkbox('topic_reply_postcount', $topic_mm['topic_reply_postcount'] )."<strong>Увеличивать счётчик сообщений?</strong>"
									     )      );
	...
	function list_current()
	{
		...
		$ADMIN->page_detail = "При помощи мульти-модерации, Вы можете создавать правила, для объединения нескольких функций модерирования, что позволит Вам с лёгкостью производить модерирование тем.";
		$ADMIN->page_title  = "Мульти-модерация тем";
		
		
		$SKIN->td_header[] = array( "Название"  , "50%" );
		$SKIN->td_header[] = array( "Редактировать"   , "25%" );
		$SKIN->td_header[] = array( "Удалить" , "25%" );

		$ADMIN->html .= $SKIN->start_table( "Текущие правила мульти-модерации" );
		...
		if ( $DB->get_num_rows() )
		{
			while ( $row = $DB->fetch_row() )
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array( 
														  ...
														  "<center><a href='{$ADMIN->base_url}&amp;act=multimod&amp;code=edit&amp;id={$row['mm_id']}'>Редактировать</a></center>",
														  "<center><a href='{$ADMIN->base_url}&amp;act=multimod&amp;code=delete&amp;id={$row['mm_id']}'>Удалить</a></center>",
												 )      );
		...
		{
			$ADMIN->html .= $SKIN->add_td_basic("<center>Мульти-модерация отсутствует</center>");
		}
		
		$ADMIN->html .= $SKIN->add_td_basic("<a href='{$ADMIN->base_url}&amp;act=multimod&amp;code=new' class='fauxbutton'>Добавить новое правило</a>", 'center', 'pformstrip');
