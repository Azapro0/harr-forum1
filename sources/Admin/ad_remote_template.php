...
class ad_settings {
	...
	function ad_settings() {
		...
		switch($IN['code'])
		{
			...
			case 'previewstate':
				$this->do_message("Окно предварительного просмотра");
				break;
...
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
	...
	function css_preview()
	{
		...
		if ( ! $set = $DB->fetch_row() )
		{
			$ADMIN->error("Невозможно произвести запрос из базы данных, об использовании этой информации.");
		}
    	...
    	$html = "<html>
    	           <head>
    	              <title>Предварительный просмотр CSS</title>
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
    	              $css
    	           </head>
    	           ...
    	           <tr>
    	            <td bgcolor='#EEEEEE' style='font-size:14px'><b>Предварительный просмотр CSS элемента '$name'<br>в стиле '{$set['css_name']}'</b></td>
    	           </tr>
    	           </table>
    	           <br>
    	           <table border='1' width='95%' cellspacing='0' cellpadding='4' align='center'>
    	           <tr>
    	            <td><b>Предварительный просмотр</b></td>
    	           </tr>
    	           <tr>
    	           	...
    	           	<table border='1' width='95%' cellspacing='0' cellpadding='4' align='center'>
    	            <tr>
    	             <td colspan='2'><b>Форма CSS определения</b></td>
    	            </tr>
    	              $def_output
    	           </table>
    	           </body>
	...
	function print_css_searchlinks()
	{
		...
		if (! $DB->get_num_rows() )
		{
			$this->do_message("Нет ни одной записи с такой строкой в шаблоне {$set['set_id']}");
		}
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Результаты поиска</b>" )  , 'title' );
	...
	function print_searchlinks()
	{
		...
		if ($search_text == "")
		{
			$this->do_message("Введите строку для поиска");
		}
		...
		if (! $DB->get_num_rows() )
		{
			$this->do_message("Нет записей с такой строкой");
		}
	...
	function print_search_box()
	{
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Введите текст для поиска</b><br>".
...
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
	...
	function print_compare_top()
	{
		...
		$content = $IN['content'] == 'orig' ? 'Оригинальный шаблон' : 'Текущий шаблон';
...
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
	...
	function macro_one()
	{
		...
		$ADMIN->html .= $SKIN->start_table( "Поиск макроса" );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Введите макрос для поиска</b><br>(ПРИМЕР: {ibf.skin.tbl_width})",
	...
	function macro_two()
	{
		...
		if ($IN['lookup'] == "")
		{
			$ADMIN->error("Вы должны ввести макрос, для поиска", 1);
		}
		...
		if ($is_valid != 1)
		{
			$ADMIN->error("Вы ввели макрос, в неправильном формате. Попробуйте снова.", 1);
		}
		...
		if ($extra != "")
		{
			if ($extra == 'member')
			{
				...
				else if (isset($GROUP[$macro]))
				{
					...
					{
						$answer = $GROUP[$macro]." (Это информация пользовательской группы)";
					}
				}
					
				$result = "Загруженная пользовательская информация";
			}
			else if ($extra == 'vars')
			{
				...
				$result = "Конфигурирование переменной (Можно установить защиту)";
			}
			else if ($extra == 'lang')
			{
				$result = "Текст языка";
		...
		$ADMIN->html .= $SKIN->start_table( "Результат поиска макроса <a href='{$SKIN->base_url}&act=rtempl&code=macro_one&suid={$IN['suid']}'>Искать снова</a>" );
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Введённый макрос</b>", $IN['lookup'] ) );
										 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Тип макроса</b>", $result )      );
										 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Результат</b><br>Может быть пустым, если нет информации", $answer )      );
	...
	function do_preview()
	{
		...
		$table = "<table width='100%' bgcolor='black' cellpadding='4' style='font-family:verdana, arial;font-size:11px;color:white'>
				  <tr>
				   <td align='center' style='font-family:verdana, arial;font-size:11px;color:white'>Группа шаблона: {$template['group_name']} : Template bit: {$template['func_name']}</td>
				  </tr>
				  <tr>
				   <td align='center' style='font-family:verdana, arial;font-size:11px;color:white'>Просмотр в виде [ <a href='{$ADMIN->base_url}&act=rtempl&code=preview&suid={$IN['suid']}&type=text' style='font-family:verdana, arial;font-size:11px;color:white'>Текста</a> | <a href='{$ADMIN->base_url}&act=rtempl&code=preview&suid={$IN['suid']}&type=html' style='font-family:verdana, arial;font-size:11px;color:white'>HTML</a> | <a href='{$ADMIN->base_url}&act=rtempl&code=preview&suid={$IN['suid']}&type=css' style='font-family:verdana, arial;font-size:11px;color:white'>HTML с CSS</a> ]</td>
				  </tr>
				  </table>
				  <br><br>
				  ";
		...
		else if($IN['type'] == 'css')
		{
			...
			@header("Content-type: text/html");
			print "<html><head><title>Предварительный просмотр</title>
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
$css_text</head><body>$table \n";
