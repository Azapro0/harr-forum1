...
class ad_settings {
	...
	function export()
	{
		...
		if ($IN['id'] == "")
		{
			$ADMIN->error("Вы должны определить ID существующего скина. Вернитесь назад и повторите попытку.");
		}
		...
		{
			$ADMIN->error("Невозможно определить местоположение директории $images_dir");
		}
		...
		{
			$ADMIN->error("Произошла ошибка при создании директории, невозможно экспортировать скин. Проверьте атрибуты директории 'archive_out'");
		}
		...
		if ( ! $DB->get_num_rows($groups) )
		{
			...
			$ADMIN->error("Неудачное экспортирование при создании шаблона: Невозможно произвести запрос информации из базы данных");
		}
		...
		$ADMIN->done_screen("Скин экспортирован<br><br>Вы можете скачать tar-архив <a href='archive_out/{$new_dir}.tar' target='_blank'>здесь</a>", "Настройка скинов", "act=sets" );
	...
	function mem_skins()
	{
		...
		$ADMIN->done_screen("Скин для пользователей обновлён", "Настройка скинов", "act=sets" );
	...
	function remove()
	{
		...
		{
			$ADMIN->error("Невозможно произвести запрос информаии об этом скине, из базы данных");
		}
		...
		{
			$ADMIN->error("Нельзя удалять скин, установленный по умолчанию. Установите по умолчанию другой скин и попробуйте снова.");
		}
	...
	function save_skin( $type='add' )
	{
		...
		if ($IN['sname'] == "")
		{
			$ADMIN->error("Вы должны определить название для этого скина");
		}
		...
		else
		{
			...
			$ADMIN->done_screen("Скин обновлён", "Настройка скинов", "act=sets" );
		}
	...
	function do_form( $type='add' )
	{
		...
		if ($IN['id'] == "")
		{
			$ADMIN->error("Вы должны указать ID существующего шаблона. Вернитесь назад и повторите попытку.");
		}
		...
		$ADMIN->page_detail = "Вы можете установить для этого скина ресурсы других установленных скинов.";
		$ADMIN->page_title  = "Настройка скинов";
		...
		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>Использовать HTML шаблоны:</b>",
													$SKIN->form_dropdown('template', $templates, $row['set_id']),
									     )      );
									     
		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>Использовать макрос:</b>",
													$SKIN->form_dropdown('macro_id', $macros, $row['macro_id']),
									     )      );
									     
		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>Использовать изображения директории:</b>",
													$SKIN->form_dropdown('img_dir', $dirs, $row['img_dir']),
									     )      );
									     
		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>Использовать стиль:</b>",
													$SKIN->form_dropdown('css', $css, $row['css_id']).'&nbsp;'.$cssextra,
									     )      );
									     
		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>Использовать шаблон:</b>",
													$SKIN->form_dropdown('wrapper', $wrappers, $row['tmpl_id']),
									     )      );
									     

		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>Скрыть от пользователей?</b><br>Применяется в том случае, если Вы устанавливаете для каждого форума свой скин",
													$SKIN->form_yes_no('hidden', $row['hidden']),
									     )      );
									     
		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>Установить этот скин по умолчанию?</b>",
	...
	function list_sets()
	{
		...
		$ADMIN->page_detail = "Здесь Вы можете добавлять, редактировать или удалять скины.";
		$ADMIN->page_title  = "Настройка скинов";
		...
		if ( $DB->get_num_rows() )
		{
			...
			$SKIN->td_header[] = array( "Скрытый"      , "5%" );
			$SKIN->td_header[] = array( "По умолчанию"     , "5%" );
			
			//+-------------------------------
		
			$ADMIN->html .= $SKIN->start_table( "Текущие используемые пользователями скины" );
			...
			while ( $r = $DB->fetch_row() )
			{
				...
				if ( is_array($forum_skins[ $r['sid'] ]) )
				{
					...
					{
						$extra = "<br>(Используется в форумах: ".implode( ",", $forum_skins[ $r['sid'] ] )." )";
					}
				}
				...
				$editlist = "<br /><b>Редактировать:</b> <a href='{$SKIN->base_url}&act=wrap&code=edit&id={$r['tmpl_id']}'>Шаблон</a>
							...
							&middot; <a href='{$SKIN->base_url}&act=style&code=colouredit&id={$r['css_id']}'>Цвета</a>
							&middot; <a href='{$SKIN->base_url}&act=image&code=edit&id={$r['macro_id']}'>Макросы</a>";
				
				$ADMIN->html .= $SKIN->add_td_row( array( "<b>".$std->txt_stripslashes($r['sname'])."</b>$extra".$editlist,
														  "<center>".$r['mcount']."</center>",
														  "<center><a href='".$SKIN->base_url."&act=sets&code=export&id={$r['uid']}'>Экспорт</a></center>",
														  "<center><a href='".$SKIN->base_url."&act=sets&code=edit&id={$r['uid']}'>Редактировать</a></center>",
														  "<center><a href='".$SKIN->base_url."&act=sets&code=remove&id={$r['uid']}'>Удалить</a></center>",
		...
		if ( $left_one > 0 or $left_two > 0 )
		{
			...
			$ADMIN->html .= $SKIN->start_table( "Неиспользуемые пользователями скины" );
			...
			while ( $r = $DB->fetch_row() )
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array( "<b>".$std->txt_stripslashes($r['sname'])."</b>$extra".$editlist,
														  "<center><a href='".$SKIN->base_url."&act=sets&code=export&id={$r['uid']}'>Экспорт</a></center>",
														  "<center><a href='".$SKIN->base_url."&act=sets&code=edit&id={$r['uid']}'>Редактировать</a></center>",
														  "<center><a href='javascript:checkdelete(\"act=sets&code=remove&id={$r['uid']}\")'>Удалить</a></center>",
														  "<center>$hidden</center>",
														  "<center>$default</center>",
												 )      );
		...
		$ADMIN->html .= $SKIN->start_table( "Создание нового скина" );
			
		//+-------------------------------
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Создать новый скин на основе...</b>" ,
										  		  $SKIN->form_dropdown( "id", $form_array)
								 )      );
		
		$ADMIN->html .= $SKIN->end_form("Создать новый скин");
		...
		$ADMIN->html .= $SKIN->start_table( "Установка скина для пользователей" );
			
		//+-------------------------------
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Для пользователей, использующих скин...</b>",
										  		  $SKIN->form_dropdown( "oid", $form_array)
								 )      );
								 
		$form_array[] = array( 'n', 'Не использовать (Установить скин, используемый по умолчанию)' );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Установить скин...</b>",
										  		  $SKIN->form_dropdown( "nid", $form_array)
								 )      );
		
		$ADMIN->html .= $SKIN->end_form("Установить");
