...
class ad_settings {
	...
	function add_splash()
	{
		...
		{
			$ADMIN->error("Невозможно определить местоположение директории загрузки - проверьте путь к директории 'uploads'");
		}
		...
		{
			$ADMIN->error("Этот файл является неправильным файлом шаблона для Invision Power Board");
		}
		...
		$wrap_name .= "Новый шаблон (Upload ID: ".substr( time(), -6 ).")";
		...
		$ADMIN->done_screen("Шаблон форума импортирован", "Настройка шаблонов форума", "act=wrap" );
	...
	function export()
	{
		...
		if ($IN['id'] == "")
		{
			$ADMIN->error("Необходимо определить ID существующего шаблона. Вернитесь назад и повторите попытку.");
		}
	...
	function save_wrapper( $type='add' )
	{
		...
		if ($IN['name'] == "")
		{
			$ADMIN->error("Необходимо определить название для этого шаблона");
		}
		
		if ($IN['template'] == "")
		{
			$ADMIN->error("Невозможно использовать пустой шаблон");
		}
		...
		{
			$ADMIN->error("Нельзя удалять тэг &lt% BOARD %>!");
		}
		
		if ( ! preg_match( "/<% COPYRIGHT %>/", $tmpl ) )
		{
			$ADMIN->error("Нельзя удалять тэг &lt% COPYRIGHT %>!");
		}
		...
		else
		{
			...
			$ADMIN->done_screen("Шаблон обновлён", "Настройка шаблонов форума", "act=wrap" );
		}
	...
	function do_form( $type='add' )
	{
		...
		{
			$code = 'doadd';
			$button = 'Создать шаблон форума';
			$row['name'] = $row['name'].".2";
		}
		else
		{
			$code = 'doedit';
			$button = 'Редактировать шаблон форума';
		}
		
		//+-------------------------------
	
		$ADMIN->page_detail = "В шаблонах Вы можете полноценно использовать HTML.";
		$ADMIN->page_title  = "Настройка шаблонов форума";
	...
	function list_wrappers()
	{
		...
		$ADMIN->page_detail = "Здесь Вы можете добавлять, редактировать или удалять шаблоны форума.<br><br>Шаблон форума, является основным шаблоном конференции, в котором Вы можете использовать HTML, например для редактирования шапки и нижнего колонтитула форума";
		$ADMIN->page_title  = "Настройка шаблонов форума";
		
		//+-------------------------------
		
		$SKIN->td_header[] = array( "Название"        , "40%" );
		$SKIN->td_header[] = array( "Использование"   , "30%" );
		...
		if ( $DB->get_num_rows() )
		{
			...
			while ( $r = $DB->fetch_row() )
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array( "<b>".stripslashes($r['name'])."</b>",
														  "<#X-{$r['tmid']}#>",
														  "<center><a href='".$SKIN->base_url."&act=wrap&code=export&id={$r['tmid']}'>Экспорт</a></center>",
														  "<center><a href='".$SKIN->base_url."&act=wrap&code=edit&id={$r['tmid']}'>Редактировать</a></center>",
			...
			if ( $DB->get_num_rows() )
			{
				...
				while ( $r = $DB->fetch_row() )
				{
					...
					$ADMIN->html .= $SKIN->add_td_row( array( "<b>".stripslashes($r['name'])."</b>",
															  "<center><a href='".$SKIN->base_url."&act=wrap&code=export&id={$r['tmid']}'>Экспорт</a></center>",
															  "<center><a href='".$SKIN->base_url."&act=wrap&code=edit&id={$r['tmid']}'>Редактировать</a></center>",
															  "<center><a href='javascript:checkdelete(\"act=wrap&code=remove&id={$r['tmid']}\")'>Удалить</a></center>",
													 )      );
		...
		$ADMIN->html .= $SKIN->start_table( "Создание нового шаблона" );
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b><u>ИЛИ</u> Выберите для загрузки, файл шаблона с Вашего компьютера</b><br>Примечание: Файл должен иметь правильный формат.",
												  $SKIN->form_upload(),
										 )      );
