...
class ad_settings {
	...
	function add_splash()
	{
		...
		{
			$ADMIN->error("Невозможно определить местоположение директории загрузок - проверьте правильность указания пути к директории 'uploads'");
		}
		...
		{
			$ADMIN->error("Невозможно открыть загруженный файл, для чтения!");
		}
		...
		{
			$ADMIN->error("Этот файл является некорректным файлом шаблона для Invision Power Board");
		}
		...
		$pack_name .= "(ID файла: ".substr( time(), -6 ).")";
		...
		$ADMIN->done_screen("Группа шаблонов успешно импортирована", "Настройка HTML шаблонов", "act=templ" );
	...
	function upload_single()
	{
		...
		{
			$ADMIN->error("Загруженный файл является некорректным файлом для этой группы шаблонов. При поиске в группе шаблонов '{$IN['group']}', найден '$found_group'");
		}
		...
		$ADMIN->done_screen("Группа шаблонов обновлена", "Настройка HTML шаблонов", "act=templ" );
	...
	function upload_form()
	{
		...
		$ADMIN->page_detail = "Тщательно перепроверьте введённую информацию перед продолжением.";
		$ADMIN->page_title  = "Загрузка файла шаблона для группы шаблонов: {$row['skname']}";
		...
		$ADMIN->html .= $SKIN->start_table("Загрузка файла шаблона для замены в: ".$skin_names[ $IN['group'] ][0]);
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Выберите файл шаблона на Вашем компьютере</b><br>Примечание: При загрузке этого файла все текущие данные будут заменены на новые. Откат назад невозможен!.",
												  $SKIN->form_upload(),
										 )      );
									     
		$ADMIN->html .= $SKIN->end_form('Загрузить файл');
										 
		$ADMIN->html .= $SKIN->end_table();
		
		$ADMIN->nav[] = array( 'act=templ' ,'Главная страница настройки шаблонов' );
	...
	function export()
	{
		...
		if ( ! $set = $DB->fetch_row() )
		{
			$ADMIN->error("Невозможно найти группу шаблонов с таким ID в базе данных. Повторите попытку.");
		}
	...
	function show_cats()
	{
		...
		{
			$author = "<br><br>Автором этого шаблона <b>'{$row['skname']}'</b> является <a href='mailto:{$row['email']}' target='_blank'>{$row['author']}</a>";
		}
		else if ($row['author'])
		{
			$author = "<br><br>Автором этого шаблона <b>'{$row['skname']}'</b> является {$row['author']}";
		}
		
		if ($row['url'])
		{
			$url = " (Сайт: <a href='{$row['url']}' target='_blank'>{$row['url']}</a>)";
		}
		
		//+-------------------------------
	
		$ADMIN->page_detail = "Ниже выберите секцию, необходимую для редактирования.<br><br><b>Скачать</b> эту HTML секцию. При использовании этой опции, Вы можете скачивать любые HTML для их дальнейшего редактирования в офлайне.<br><b>Загрузка</b> HTML для этой секции. Вы можете заменить секцию шаблона, на уже отредактированную.$author $url";
		$ADMIN->page_title  = "Редактирование группы шаблона";
		...
		$SKIN->td_header[] = array( "Название категории скина"   , "40%" );
		$SKIN->td_header[] = array( "Опции"          , "20%" );
		$SKIN->td_header[] = array( "Управление"                , "30%" );
		$SKIN->td_header[] = array( "Кол-во частей"                , "10%" );
		...
		$ADMIN->html .= "<script language='javascript'>
						 function pop_win(theUrl) {
						 	...
						 	window.open('{$SKIN->base_url}&'+theUrl,'Предпросмотр','width=400,height=450,resizable=yes,scrollbars=yes');
						 }
						 </script>";
						 
		$qc = $SKIN->form_dropdown( 'entry', array(
													0 => array( 'boardheader', 'Редактирование шапки форума'             ),
													1 => array( 'catforum'   , 'Редактирование таблиц форумов и категорий'   ),
													2 => array( 'topiclist'  , 'Редактирование страницы тем и таблиц' ),
													3 => array( 'postlist'   , 'Редактирование отображения сообщений и таблиц'        ),
								  )			     );
												  
						 
		$quick_click = "<form action='{$SKIN->base_url}&act=templ&code=find_pop&id={$IN['id']}' method='post'>
						<div align='right'>Быстрый&nbsp;переход&nbsp;&nbsp;{$qc}&nbsp;<input type='submit' value='ОК' /></div></form>
					   ";
		
		$ADMIN->html .= $SKIN->start_table("Шаблон: ".$row['skname'], $quick_click);
		...
		while ( $group = $DB->fetch_row($all_cats) )
		{
			...
			$expand = 'Раскрыть для редактирования';
			$eid    = $group['suid'];
			$exp_content = "";
			...
			{
				$name .= " (Нестандартная группа)";
				$desc = "<br>Эта группа поставляется не в комплекте со стандартной установкой Invision Power Board. Описание для этой группы отсутствует.";
			}
			...
			if ($IN['expand'] == $group['suid'])
			{
				...
				if ( $DB->get_num_rows($new_q) )
				{
					$exp_content .= $SKIN->add_td_basic( "<script type='text/javascript'>
														  ...
														  <tr>
														   <td align='left' colspan='2' class='catrow2'><a style='font-weight:bold;font-size:12px;color:#000033' href='{$SKIN->base_url}&act=templ&code=edit&id={$IN['id']}&expand=' title='Развернуть' alt='Развернуть'>$name</a></td>
														   <td colspan='3' bgcolor='#FFFFFF'>&nbsp;</td>
														  <tr>
														   <td width='5%' class='catrow2' align='center'><input type='checkbox' onclick='checkall()' name='allbox' value='1' /></td>
														   <td width='25%' class='catrow2'>Редактирование секций</td>
														   <td width='20%' class='catrow2' align='center'>Кол-во символов</td>
														   <td width='20%' class='catrow2' align='center'>Редактировать</td>
														   <td width='30%' class='catrow2' align='center'>Опции предпросмотра</td>
														  </tr>
														  <!--CONTENT-->
														  <tr>
														   <td colspan='5'  class='catrow2' align='center'><input type='submit' value='Редактировать выбранное' /></td>
														  </tr>
					...
					foreach( $sec_arry as $id => $sec )
					{
						...
						$temp .= "
									<tr>
									 ...
									 <td width='10%' class='tdrow1' align='center'><a href='{$SKIN->base_url}&act=templ&code=edit_bit&suid={$sec['suid']}&expand={$group['suid']}&type=single'>Редактировать</a></td>
									 <td width='40%' class='tdrow1' align='center' nowrap>(<a href='javascript:pop_win(\"act=rtempl&code=preview&suid={$sec['suid']}&type=html\")'>HTML</a> | <a href='javascript:pop_win(\"act=rtempl&code=preview&suid={$sec['suid']}&type=text\")'>Текст</a> | <a href='javascript:pop_win(\"act=rtempl&code=preview&suid={$sec['suid']}&type=css\")'>В виде CSS</a>)</td>
									</tr>
								";
					}
			...
			else
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array( 
															...
															"<center><a href='{$SKIN->base_url}&act=templ&code=download&setid={$group['set_id']}&group={$group['group_name']}' title='Скачать HTML файл этой секции, для редактирования в офлайне'>Скачать</a> | <a href='{$SKIN->base_url}&act=templ&code=upload&setid={$group['set_id']}&group={$group['group_name']}' title='Загрузить отредактированный HTML файл в эту секцию, для замены'>Загрузить</a></center>",
		...
		$ADMIN->nav[] = array( 'act=templ' ,'Главная страница настройки шаблонов' );
		$ADMIN->nav[] = array( '' ,'Настройка группы шаблона "'.$row['skname'].'"' );
	...
	function edit_info()
	{
		...
		$ADMIN->done_screen("Информация шаблона обновлена", "Настройка HTML шаблонов", "act=templ" );
	...
	function tools()
	{
		...
		if ($IN['id'] == "")
		{
			$ADMIN->error("Вы должны выбрать правильный файл скина, для выполнения этого действия.");
		}
	...
	function tools_build_tmpl()
	{
		...
		{
			$ADMIN->error("Эта группа шаблонов использует безопасный режим для шаблонов и PHP скин файлы у этого скина отсутствуют. Поэтому, нет необходимости ресинхронизации этого скина.");
		}
		
		
		if ( ! is_readable($skin_dir) )
		{
			$ADMIN->error("Невозможно произвести запись в директорию '$skin_dir'. Проверьте атрибуты и при необходимости установите через FTP - CHMOD на 0777. Скрипт форума не в состоянии сделать это самостоятельно.");
		}
		...
		if ( is_dir($skin_dir) )
		{
				...
				while (($filename = readdir($handle)) !== false)
				{
					if (($filename != ".") && ($filename != ".."))
					{
						...
						if ( preg_match( "/\.php$/", $filename ) )
						{
							...
							{
								$errors[] = "Невозможно открыть файл $filename для чтения. Файл пропущен...";
								continue;
							}
							...
							if ( ! preg_match( "/\n/", $fdata ) )
							{
								$errors[] = "Невозможно найти конечные строки в файле $filename. Файл пропущен...";
								continue;
							}
			...
			{
				$ADMIN->error("Невозможно открыть директорию $skin_dir для чтения!");
			}
		}
		else
		{
			$ADMIN->error("$skin_dir не является директорией. Проверьте переменную \$root_path в файле admin.php");
		}
		
		$ADMIN->done_screen("Отредактированные шаблоны обновлены из исходных PHP файлов", "Настройка HTML шаблонов", "act=templ" );
		
		if (count($errors > 0))
		{
			$this->html .= $SKIN->start_table("Внимание, Ошибка!");
	...
	function add_templates()
	{
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			if (SAFE_MODE_ON == 1)
			{
				$ADMIN->error("Обнаружен безопасный режим. Необходимо включить 'Безопасный режим скинов'. Для его включения, зайдите в меню 'Системные настройки' и выберите там подменю 'Главная конфигурация'.");
			}
		
			if ( ! is_writeable(ROOT_PATH.'Skin') )
			{
				$ADMIN->error("Невозможно произвести запись в директорию 'Skin'. Проверьте атрибуты, при необходимости установите через FTP - CHMOD на 0777 и повторите попытку.");
			}
			...
			{
				$ADMIN->error("Невозможно определить местоположение оригинального файла шаблона для копирования. Проверьте данные и повторите попытку.");
			}
		...
		if ( ! $row = $DB->fetch_row() )
		{
			$ADMIN->error("Невозможно произвести запрос из базы данных об этой группе шаблона.");
		}
		...
		$ADMIN->done_screen("Новый шаблон установлен", "Настройка HTML шаблонов", "act=templ" );
	...
	function remove()
	{
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			{
				$ADMIN->error("Невозможно удалить файлы шаблона. Проверьте атрибуты и установите необходимый CHMOD для этого действия.");
			}
	...
	function find_popular()
	{
		...
		if ( $id < 1 )
		{
			$ADMIN->error("Ты что творишь, мужик");
		}
	...
	function edit_bit()
	{
		...
		$ADMIN->page_detail = "Вы можете редактировать HTML в этом шаблоне.";
		$ADMIN->page_title  = "Редактирование шаблона";
		...
		else
		{
 		...
 		if ( count($ids) < 1 )
 		{
 			$ADMIN->error("Не выбран ни один id. Вернитесь назад и исправьте ошибку, перед продолжением.");
 		}
		...
		foreach( $sec_arry as $id => $template )
		{
			...
			$ADMIN->html .= $SKIN->start_table( "Шаблон: ". $template['easy_name'] );
			
			$ADMIN->html .= $SKIN->add_td_basic( "<input type='button' value='Поиск макроса' id='editbutton' title='Просмотр макроса' onClick='pop_win(\"code=macro_one&suid={$template['suid']}\", \"MacroWindow\", 400, 200)'>".
												 "&nbsp;<input type='button' value='Сравнение' id='editbutton' title='Сравнение оригинальной версии с отредактированной' onClick='pop_win(\"act=rtempl&code=compare&suid={$template['suid']}\", \"CompareWindow\", 500,400)'>".
												 "&nbsp;<input type='button' value='Восстановление' id='editbutton' title='Восстановление оригинального неотредактированного шаблона' onClick='restore(\"{$template['suid']}\",\"{$IN['expand']}\")'>".
												 "&nbsp;<input type='button' value='Просмотр оригинала' id='editbutton' title='Просмотр HTML неотредактированного шаблона' onClick='pop_win(\"act=rtempl&code=preview&suid={$template['suid']}&type=html\", \"OriginalPreview\", 400,400)'>".
												 "&nbsp;<input type='button' value='Поиск' id='editbutton' title='Поиск строк в шаблоне' onClick='pop_win(\"act=rtempl&code=search&suid={$template['suid']}&type=html\", \"Search\", 500,400)'>".
												 "&nbsp;<input type='button' value='Размер окна редактора' id='editbutton' title='Изменение размера окна редактора' onClick=\"pop_win('&act=prefs', 'prefs', '300', '100')\">",
												 "center", "catrow");
												 
			$ADMIN->html .= $SKIN->add_td_basic( "<b>Показать HTML код для:&nbsp;".
		...
		$ADMIN->html .= "<div class='tableborder'><div class='catrow2' align='center'><input type='submit' value='Обновить шаблоны' /></div></div></form>";
	...
	function do_edit()
	{
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
		   ...
		   {
			   $ADMIN->error("Невозможно произвести запись в '$phpskin'. Проверьте атрибуты и при необходимости установите через FTP - CHMOD на 0777. Скрипт форума не в состоянии сделать это самостоятельно.");
		   }
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			{
				$ADMIN->error("Невозможно сохранить информацию в $phpskin. Проверьте правильность CHMOD атрибута.");
			}
		
		}
		
		$ADMIN->done_screen("Файлы шаблона обновлены", "Настройка шаблонов группы: {$row['skname']}", "act=templ&code=edit&id={$template['set_id']}" );
	...
	function do_form()
	{
		...
		$ADMIN->page_detail = "Ниже выберите секцию, которую Вы хотите отредактировать.";
		$ADMIN->page_title  = "Редактирование данных шаблона";
		...
		$ADMIN->html .= $SKIN->start_table( "Редактирование информации шаблона" );
		
									     
		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>Название группы шаблона</b>",
		...
		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>E-mail автора шаблона:</b>",
													$SKIN->form_input('email', $row['email']),
										 )      );
										 
		$ADMIN->html .= $SKIN->add_td_row( array( 
													"<b>Сайт автора шаблона:</b>",
													$SKIN->form_input('url', $row['url']),
										 )      );
									     
		
									     
		$ADMIN->html .= $SKIN->end_form("Сохранить информацию");
	...
	function list_current()
	{
		...
		$ADMIN->page_detail = "В шаблонах скина содержатся все HTML, используемые на форуме.
							   <br>
							   Здесь Вы можете создавать новые шаблоны или редактировать HTML в текущих, установленных шаблонах.";
		$ADMIN->page_title  = "Настройка HTML шаблонов";
		
		//+-------------------------------
		
		$SKIN->td_header[] = array( "Название"        , "30%" );
		$SKIN->td_header[] = array( "Использование"   , "20%" );
		$SKIN->td_header[] = array( "Редактирование&nbsp;свойств"    , "20%" );
		$SKIN->td_header[] = array( "Настройка HTML"  , "20%" );
		...
		if ( $DB->get_num_rows() )
		{
			...
			$ADMIN->html .= $SKIN->start_table( "Текущие используемые шаблоны" );
			...
			while ( $r = $DB->fetch_row() )
			{
				...
				$ADMIN->html .= $SKIN->add_td_row( array( "<b>".stripslashes($r['skname'])."</b><br>[ <a href='{$SKIN->base_url}&act=templ&code=export&id={$r['skid']}' title='Скачать полный комплект шаблона'>Экспорт</a> ]",
														  "<#X-{$r['skid']}#>",
														  //"<center><a href='".$SKIN->base_url."&act=templ&code=export&id={$r['skid']}'>Скачать</a></center>",
														  "<center><a href='".$SKIN->base_url."&act=templ&code=dedit&id={$r['skid']}' title='Редактирование названия шаблона'>Редактировать свойства</a></center>",
														  "<center><a href='".$SKIN->base_url."&act=templ&code=edit&id={$r['skid']}' title='Редактирование, загрузка и скачивание'>Настройка HTML</a></center>",
		...
		if ( count($used_ids) > 0 )
		{
			...
			if ( $DB->get_num_rows() )
			{
				...
				$ADMIN->html .= $SKIN->start_table( "Текущие неиспользуемые шаблоны" );
				...
				while ( $r = $DB->fetch_row() )
				{
					...
					$ADMIN->html .= $SKIN->add_td_row( array( "<b>".stripslashes($r['skname'])."</b><br>[ <a href='{$SKIN->base_url}&act=templ&code=export&id={$r['skid']}' title='Скачать целиком эту группу шаблонов'>Экспортировать</a> ]",
															  //"<center><a href='".$SKIN->base_url."&act=templ&code=export&id={$r['skid']}'>Скачать</a></center>",
															  "<center><a href='".$SKIN->base_url."&act=templ&code=dedit&id={$r['skid']}'>Редактировать свойства</a></center>",
															  "<center><a href='".$SKIN->base_url."&act=templ&code=edit&id={$r['skid']}' title='Редактирование, загрузка и скачивание'>Настройка HTML</a></center>",
															  "<center><a href='javascript:checkdelete(\"act=templ&code=remove&id={$r['skid']}\")'>Удалить</a></center>",
													 )      );
		...
		$ADMIN->html .= $SKIN->start_table( "Создание новой группы шаблонов" );
			
		//+-------------------------------
		
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Создать новый шаблон на основе...</b>" ,
										  		  $SKIN->form_dropdown( "id", $form_array)
								 )      );
								 
		$ADMIN->html .= $SKIN->add_td_row( array( "<b><u>ИЛИ</u> Выберите файл на Вашем компьютере для загрузки</b><br>Примечание: Это должно быть группой шаблонов.",
												  $SKIN->form_upload(),
										 )      );
		
		$ADMIN->html .= $SKIN->end_form("Создать новый шаблон");
		...
		$SKIN->td_header[] = array( "Средство"  , "50%" );
		$SKIN->td_header[] = array( "загрузка шаблона"  , "50%" );
		
		$extra = "";
		
		if ( SAFE_MODE_ON == 1)
		{
			$extra = "<br><span id='detail'>ВНИМАНИЕ: Обнаружен безопасный режим. Некоторые функции не будут работать.</span>";
		}
		
		$ADMIN->html .= $SKIN->start_table( "Средства шаблонов".$extra );
			
		//+-------------------------------
		
		$ADMIN->html .= $SKIN->add_td_row( array( $SKIN->form_dropdown( "tool",
																		array(
																				1 => array( 'tmpl', 'Ресинхронизировать базу шаблонов из PHP файлов скина'   ),
																			 )
												                      ) ,
										  		  $SKIN->form_dropdown( "id", $form_array)
								 )      );
		
		$ADMIN->html .= $SKIN->end_form("Выполнить действие");
	...
	function process_upload($raw, $setid, $group, $isnew=0)
	{
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			if ($isnew == 1)
			{
				...
				if ( ! file_exists($skin_dir) )
				{
					...
					{
						$DB->query("DELETE FROM ibf_tmpl_names WHERE skid='$setid'");
						$ADMIN->error("Невозможно создать директорию '$skin_dir'. Проверьте атрибуты CHMOD и повторите попытку.");
					}
					else
					{
						@chmod($skin_dir, 0777);
		...
		if ($INFO['safe_mode_skins'] != 1)
		{
			...
			else
			{
				...
				$errors[] = "Невозможно сохранить информацию в $phpskin. Проверьте правильность CHMOD атрибута.";
			}
