...
class ad_stats {
	...
	function show_views()
	{
		...
		$ADMIN->page_title = "Результаты статистики";
		
		$ADMIN->page_detail = "Статистика количества просмотров тем";
		
		//+---------------------
		
		if ( ! checkdate($IN['to_month']   ,$IN['to_day']   ,$IN['to_year']) )
		{
			$ADMIN->error("Неправильная установка даты в 'Конечная дата:'. Проверьте установки и повторите попытку.");
		}
		
		if ( ! checkdate($IN['from_month'] ,$IN['from_day'] ,$IN['from_year']) )
		{
			$ADMIN->error("Неправильная установка даты в 'Начальная дата:'. Проверьте установки и повторите попытку.");
		}
		...
		$SKIN->td_header[] = array( "Результат"  , "50%" );
		$SKIN->td_header[] = array( "Просмотров"   , "10%" );
		
		//+-------------------------------
		
		$ADMIN->html .= $SKIN->start_table( "Количество просмотров тем"
										    ." (с {$human_from_date['mday']} {$this->month_names[$human_from_date['mon']]} {$human_from_date['year']} по"
		...
		{
			$ADMIN->html .= $SKIN->add_td_basic( "Нет результатов", "center" );
		}
	...
	function result_screen($mode='reg')
	{
		...
		if ($mode == 'reg')
		{
			...
			$ADMIN->page_detail = "Статистика количества зарегистрированных пользователей. (Примечание: Время указано в GMT)";
		}
		else if ($mode == 'topic')
		{
			...
			$ADMIN->page_detail = "Статистика количества новых тем. (Примечание: Время указано в GMT)";
		}
		else if ($mode == 'post')
		{
			...
			$ADMIN->page_detail = "Статистика количества новых сообщений. (Примечание: Время указано в GMT)";
		}
		else if ($mode == 'msg')
		{
			$table     = 'Статистика отправленных PM';
			
			$sql_table = 'ibf_messages';
			$sql_field = 'msg_date';
			
			$ADMIN->page_detail = "Статистика количества приватных писем. (Примечание: Время указано в GMT)";
		}
		...
		$SKIN->td_header[] = array( "Дата"    , "20%" );
		$SKIN->td_header[] = array( "Результат"  , "70%" );
		$SKIN->td_header[] = array( "Кол-во"   , "10%" );
	...
	function main_screen($mode='reg')
	{
		...
		$ADMIN->page_title = "Центр статистики";
		
		$ADMIN->page_detail = "Ниже выберите дату и другие установки.<br>Примечание: Статистика генерируется из базы данных на настоящее время. Удалённые темы, сообщения и т.д. не считываются.";
		...
		else if ($mode == 'views')
		{
			...
			$table     = 'Статистика просмотра тем';
		}
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Начальная дата</b>" ,
		...
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Конечная дата</b>" ,
		...
		if ($mode != 'views')
		{
			$ADMIN->html .= $SKIN->add_td_row( array( "<b>Шкала времени</b>" ,
													  $SKIN->form_dropdown( "timescale" , array( 0 => array( 'daily', 'Дневная'), 1 => array( 'weekly', 'Недельная' ), 2 => array( 'monthly', 'Месячная' ) ) )
											 )      );
		}
						     
		$ADMIN->html .= $SKIN->add_td_row( array( "<b>Сортировка результата</b>" ,
												  $SKIN->form_dropdown( "sortby" , array( 0 => array( 'asc', 'По возрастанию - старые даты в начале'), 1 => array( 'desc', 'По убыванию - новые даты в начале' ) ), 'desc' )
									     )      );
									     									     
		$ADMIN->html .= $SKIN->end_form("Показать");
