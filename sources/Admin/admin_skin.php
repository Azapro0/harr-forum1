...
class admin_skin {
	...
	{
		return "( <a href='#' onClick=\"window.open('{$this->base_url}&act=quickhelp&id=$help','Помощь','width=250,height=400,resizable=yes,scrollbars=yes'); return false;\">$text</a> )";
				...
				<!--
					...
					function restore(suid, expand)
					{
      				...
      				function edit_box_size(cols, rows)
      				{
      					...
      					userCols = prompt(\"Введите кол-во колонок для текстовой области (ширина)\", cols);
						if ( (userCols != null) && (userCols != \"\") )
						{
							userRows = prompt(\"Введите кол-во рядов для текстовой области (высота)\", rows);
							if ( (userRows != null) && (userRows != \"\") )
							{
								...
								alert('Настройки редактора обновлены.\\nИзменения вступят в силу при загрузке следующей страницы редактирования');
							}
							else
							{
								alert('Вы должны ввести значение кол-во рядов');
							}
						}
						else
						{
							alert('Вы должны ввести значение кол-во колонок');
						}
					}
		...
		return "
				...
				function no_specialchars(type) {
				  ...
				  if (type == 'templates')
				  {
				  	...
				  	name = 'Название группы шаблнонов';
				  }
				  
				  if (type == 'images')
				  {
				  	var field = document.theAdminForm.setname;
				  	name = 'Название группы изображений и макросов';
				  }
				  ...
				  if (ok == 0)
				  {
				  	alert('Неправильный ввод для: ' + name + ', можно использовать только числовые, текстовые и следующие спец. символы.\\n. ( ) : ; ~ + - _');
				  	return false;
				  } else {
				  	return true;
				  }
	...
	{
		global $IN, $INFO;
		return "<a href='#' title=\"Переход к странице...\" onclick=\"multi_page_jump('$ub',$tp,$pp);\">Страниц:</a>";
	}
	...
	function build_group_perms( $read='*', $write='*', $reply='*', $upload='*' ) {
		...
		$html .= $this->add_td_basic( "ОБЩИЙ: Полный доступ для всех существующих и будущих масок", "left", "pformstrip" );		
		...
		$html .= $this->add_td_row( array(   "<b>Доступ для всех существующих и будущих масок</b>",
		...
		$html .= $this->add_td_basic( "ИЛИ: Установить доступы для каждой маски, ниже", "left", "pformstrip" );
...
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
				 ...
				 <tr>
				  <td><a href='{$this->base_url}&act=menu&show=all' target='menu'>Раскрыть меню</a> &middot; <a href='{$this->base_url}&act=menu&show=none' target='menu'>Свернуть меню</a></td>
				  <td align='right'><a href='{$this->base_url}&act=index' target='body'>Главная страница АЦ</a> &middot; <a href='{$INFO['board_url']}/index.{$INFO['php_ext']}' target='_blank'>Главная страница форума</a></td>
				 </tr>
				 </table>
				 <!--NAV-->
	...
	{
		return "\n<div id='nav'><img src='html/sys-img/nav.gif' border='0' title='Навигация админцентра'>&nbsp;$links</div>\n";
	}
	...
	function print_foot() {
		...
		return "<br />
				...
				<div class='copy' align='center'>Invision Power Board &copy 2003 <a href='http://www.invisionpower.com' target='_blank'>IPS, Inc.</a> <a href='http://www.script-php.ru/script_foruma/' target='_blank'>скрипт форума</a></div>
				 </body>
				 </html>";
	}
...
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
				 ...
				 <div class='tableborder'>
				  <div class='plain'>
				   <img src='{$this->img_url}/item.gif' border='0' alt='' valign='absmiddle'>&nbsp;<a href='{$this->base_url}&act=menu&show=all' target='menu'>Раскрыть</a> &middot; <a href='{$this->base_url}&act=menu&show=none' target='menu'>Свернуть</a> Меню
				   <br /><img src='{$this->img_url}/item.gif' border='0' alt='' valign='absmiddle'>&nbsp;<a href='{$this->base_url}&act=index' target='body'>Админцентр</a> &middot; <a href='{$INFO['board_url']}/index.{$INFO['php_ext']}' target='body'>Форум</a> Гл.стр.
				   <br /><img src='{$this->img_url}/item.gif' border='0' alt='' valign='absmiddle'>&nbsp;<a href='{$this->base_url}&act=ips&code=docs' target='body' style='text-decoration:none'>Документация IPB</a>
				   <br /><img src='{$this->img_url}/item.gif' border='0' alt='' valign='absmiddle'>&nbsp;<a href=\"javascript:pop_win('&act=prefs', 'prefs', 400, 200 )\"  style='text-decoration:none'>Настройки админцентра</a>
				  </div>
				 </div>
				 <br />
				";
	...
	function menu_cat_expanded($name="", $links="", $id = "") {
		...
		return "<a name='cat$id'></a>
				<div class='tableborder'>
				  <div class='cattitle'>
				    <a href='{$this->base_url}&act=menu&show={$IN['show']}&out=$id' target='menu'><img src='{$this->img_url}/minus.gif' border='0' alt='Свернуть категорию' title='Свернуть категорию'></a>
	...
	function menu_cat_collapsed($name="", $id = "", $desc="") {
		...
		return "<div class='tableborder'>
				  <div class='cattitle'>
				    <a href='{$this->base_url}&act=menu&show=,{$IN['show']},$id#cat$id' target='menu'><img src='{$this->img_url}/plus.gif' border='0' alt='Развернуть категорию' title='Развернуть категорию'></a>
	...
	function frame_set() {
		...
		$frames = "<html>
		   			 <head><title>Центр Администрирования Invision Power Board</title>
<META http-equiv='Content-Type' content='text/html;charset=windows-1251'>
</head>
